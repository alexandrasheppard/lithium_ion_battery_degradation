...
bat_Pmax = ['025MW', '025MW', '05MW', '05MW', '1MW', '05MW', '1MW', '2MW', '2MW', '4MW'] #,
bat_Emax = ['025MWh', '05MWh', '05MWh', '1MWh',  '1MWh', '2MWh', '2MWh', '2MWh', '4MWh', '4MWh'] #
...
for i in range(len(bat_Pmax)):
    ikey = f'{bat_Emax[i]}_{bat_Pmax[i]}'
    for k in time_resolutions:
        # keys = keys.append(ikey)
        store_d[(ikey,k)] = pd.read_csv(f'.\Results\{ikey}_{k}_SOC.csv')
...
ax = sns.heatmap(final_deg_size_time, annot=True, fmt='.4f', yticklabels=time_resolution_names) #xticklabels=keys,
ax.set(xlabel='Battery size/type', ylabel='Time resolution of power settlement (minutes)', title='Remaining capacity after 15 years operation')
